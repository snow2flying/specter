...
- **Native QUIC path migration primitives (`src/transport/h3/path.rs`)**: Added `QuicConnectionIdInventory` for RFC9000 § 5.1 connection-ID inventory (locally issued and peer-issued CIDs tracked by sequence number, `active_connection_id_limit` enforced per RFC9000 § 18.2, NEW_CONNECTION_ID frames validated per § 19.15 including the retire_prior_to constraint and the active-limit check, RETIRE_CONNECTION_ID frames processed per § 19.16, an outbound retire queue surfaced via `drain_pending_peer_retires`, and a peer-active CID promotion gate that rejects retired sequences); `QuicAntiAmplificationLimit` for the RFC9000 § 8.1 per-path 3x send budget (received-byte counter, sent-byte counter, `remaining_send_budget`, `may_send`, and `mark_validated` to remove the cap when the path validates); and `QuicPathSet`/`QuicPath`/`QuicPathState` for RFC9000 § 9 per-address migration state (Probing on new addresses, Validating once PATH_CHALLENGE is issued, Validated on matching PATH_RESPONSE which also marks the anti-amplification cap removed, Primary after promotion which demotes the previous primary to Abandoned). Coverage lives in `src/transport/h3/path.rs::tests` (18 unit tests for anti-amplification budget arithmetic, sequence-number validation, retire_prior_to handling, active-limit enforcement, primary-path validation, probing-path observation from new addresses, challenge/response promotion, and promote-to-primary demotion). Driver integration (NEW_CONNECTION_ID emission after handshake completion, active peer CID swap on path promotion, outbound builder gating on the anti-amplification budget, and server migration lifecycle) is tracked separately.
...
- **RFC 9220 close/FIN and mixed low-level comparator rows**: Added `quiche_direct_rfc9220_tunnel_close`, `tokio_quiche_rfc9220_tunnel_close`, `quiche_direct_rfc9220_tunnel_mixed`, and `tokio_quiche_rfc9220_tunnel_mixed` benchmark specs, row helpers, local fixture dispatch, direct measurement flags, and fixture tests. The new `docs/benchmarks/native-h3-vs-rust-clients/2026-05-25-rfc9220-n100-plus-close-and-mixed-comparators.json` artifact merges the existing n=100 echo-gate proof with n=30 low-level close/FIN and slow-consumer mixed comparator rows, keeps `rfc9220_tunnel_superiority_gate.pass = true`, and emits zero `fixture_events`; full tunnel-suite superiority remains unclaimed because the dedicated gate still covers echo rows only and mixed-workload optimization/evidence remains open.
