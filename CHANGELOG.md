...
- **Native H3 capacity snapshots**: Public H3 streaming response bodies now expose `Body::h3_capacity()` for buffered chunk/byte pressure, and RFC 9220 tunnels expose `H3Tunnel::capacity()` for outbound/inbound byte-budget pressure. These surfaces let callers inspect native H3 response and tunnel backpressure without reaching into driver internals; cross-protocol H1/H2/H3 policy unification remains a separate API-design task.
