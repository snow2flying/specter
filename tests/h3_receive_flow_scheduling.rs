...
#[test]
fn native_h3_driver_does_not_drain_tunnel_data_ahead_of_queued_commands() {
    let driver =
        std::fs::read_to_string("src/transport/h3/native_driver.rs").expect("native driver source");
    let drive_loop = driver
        .split("async fn drive_loop")
        .nth(1)
        .expect("native H3 driver must have drive_loop")
        .split("fn has_pending_work")
        .next()
        .expect("drive loop section");
    let scheduled_continue = drive_loop
        .split("if sent_scheduled_data")
        .nth(1)
        .expect("drive loop must have scheduled-send continuation guard")
        .split("continue;")
        .next()
        .expect("scheduled-send continuation guard body");

    assert!(
        scheduled_continue.contains("self.command_rx.is_empty()"),
        "native H3 driver must service queued request/tunnel-open commands instead of draining all RFC9220 tunnel DATA first"
    );
    assert!(
        scheduled_continue.contains("tokio::task::yield_now().await"),
        "native H3 driver must yield between scheduled DATA turns so same-origin request tasks can enqueue commands before the next tunnel DATA flush"
    );
}

#[test]
fn native_h3_driver_batches_ready_commands_before_flushing_tunnel_data() {
    let driver =
        std::fs::read_to_string("src/transport/h3/native_driver.rs").expect("native driver source");
    let command_branch = driver
        .split("command = self.command_rx.recv()")
        .nth(1)
        .expect("drive loop must receive public commands")
        .split("recv = recv_from_with_ecn")
        .next()
        .expect("command branch section");
    let send_tunnel_data = driver
        .split("fn send_tunnel_data")
        .nth(1)
        .expect("native H3 driver must enqueue tunnel data")
        .split("async fn flush_scheduled_send_work")
        .next()
        .expect("send_tunnel_data section");
    let ready_drain = driver
        .split("fn drain_ready_tunnel_data_commands")
        .nth(1)
        .expect("native H3 driver must drain ready commands")
        .split("async fn queue_flow_control_blocked_command")
        .next()
        .expect("ready command drain section");

    assert!(
        command_branch.contains("drain_ready_tunnel_data_commands()")
            && drive_loop.contains("process_pending_commands().await?")
            && drive_loop
                .find("process_pending_commands().await?")
                .expect("pending commands processed")
                < drive_loop
                    .find("flush_scheduled_send_work().await?")
                    .expect("scheduled send flush"),
        "after one command wakes the driver, it must batch already-ready commands so request commands queued behind tunnel DATA are not delayed by per-DATA flushes"
    );
    assert!(
        ready_drain.contains("try_recv()"),
        "ready command batching must drain already-buffered commands without blocking the socket path"
    );
    assert!(
        !send_tunnel_data.contains("flush_scheduled_send_work().await?"),
        "SendTunnelData command handling must enqueue only; the fair scheduler should flush DATA after ready request commands have been serviced"
    );
}
