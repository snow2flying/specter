...
That gate is explicitly for HTTP/3 request/response workloads. The same artifact includes Specter RFC 9220 WebSocket-over-H3 tunnel echo, close/FIN, mixed slow-consumer rows, and measured low-level `quiche`/`tokio-quiche` raw tunnel comparator rows; n=100 tunnel samples are persisted separately in [`2026-05-24-rfc9220-n100.json`](docs/benchmarks/native-h3-vs-rust-clients/2026-05-24-rfc9220-n100.json). This is still not a published RFC 9220 tunnel superiority claim until a dedicated tunnel gate and third-party close/FIN plus slow-consumer comparator rows exist. `quinn_transport` and `s2n_quic_transport` are separate QUIC transport-only evidence, not H3 HTTP comparator rows. Native QUIC production hardening remains active work for broader recovery soak/backoff validation, socket-level ECN receive reporting into ACK_ECN mark observation, PMTU/path probing, full per-address path migration, browser ACK parity, and safe 0-RTT request replay policy.
