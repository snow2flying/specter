...
- **RFC9220 WebSocket-over-H3:** correctness/API exists as a raw byte tunnel. The same-fixture proof now includes Specter local rows for echo, client DATA+FIN/server FIN, a slow-consumer tunnel plus concurrent H3 streaming workload, measured low-level `quiche`/`tokio-quiche` raw tunnel comparator rows, and a persisted n=100 sample artifact with a passing dedicated echo tunnel superiority gate. A full tunnel-suite superiority claim still waits on third-party close/FIN plus slow-consumer comparator rows.
...
- RFC9220/WebSocket-over-H3 now has n=100 echo/close/mixed samples for Specter, n=100 low-level `quiche`/`tokio-quiche` echo comparator rows, and a dedicated RFC9220 echo tunnel gate; third-party close/FIN and slow-consumer comparator rows remain open for a full tunnel-suite claim.
...
- Specter RFC9220 local tunnel throughput/latency rows, n=100 p99-scale samples, low-level `quiche`/`tokio-quiche` raw tunnel echo comparator rows, and the dedicated echo tunnel superiority gate are no longer pending; only third-party close/FIN plus slow-consumer comparator rows and unsupported higher-level client capability rows remain open.
...
1. **RFC9220 full tunnel-suite superiority proof:** Specter local tunnel echo, close/FIN, slow-consumer mixed rows, low-level `quiche`/`tokio-quiche` raw tunnel echo comparator rows, and a dedicated echo tunnel superiority gate are persisted at n=100; third-party close/mixed comparator coverage remains missing.
...
5. **Expand RFC9220 benchmark rows**
   - Raw byte tunnel: echo RTT, close/FIN latency, and slow-consumer mixed workload are now measured for Specter locally at n=100; the dedicated echo tunnel superiority gate passes, and the next gap is third-party close/mixed comparator rows.
