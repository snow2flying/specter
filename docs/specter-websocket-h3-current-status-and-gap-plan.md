...
- **Native QUIC production readiness:** still not production-complete; PTO send-time tracking, ACK-driven RTT/PTO estimator updates, client Initial/Handshake plus server Initial/Handshake CRYPTO PTO retransmission, client application-space PTO timer/retransmit, server application ACK-driven recovery plus mock/same-fixture PTO STREAM retransmit wake, event-level peer-close draining, bounded client/server CONNECTION_CLOSE replay/suppression, 1-RTT key-update handling, ACK_ECN frame/counter validation and CE-driven congestion response, Retry/VN client-handshake handling, client PATH_CHALLENGE/PATH_RESPONSE token lifecycle, H3Client-level native session-cache wiring, TLS session replay, and 0-RTT accept/reject status reporting exist, but broader recovery soak/backoff policy validation, ECN socket marking/reporting plus ACK_ECN generation, PMTU probing, per-address path migration, and safe end-to-end 0-RTT request replay policy remain gaps.
...
- RFC9220 tunnel inbound backpressure is no longer item-count-only: native H3 uses an unbounded driver-to-handle event channel gated by per-tunnel receive byte credit, queues additional DATA when the byte budget is exhausted, and releases credit when public tunnel reads consume DATA.
