...
1. Capture browser ACK behavior and map it to Specter's fingerprint knobs.
2. Add capture-derived H3/TLS presets where backend control is sufficient.
3. Decide whether unified cross-protocol capacity policy and WebSocket ergonomics are product requirements before adding more API surface.
