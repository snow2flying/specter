...
- **RFC9220 WebSocket-over-H3:** correctness/API exists as a raw byte tunnel. The same-fixture proof now includes Specter local rows for echo, client DATA+FIN/server FIN, a slow-consumer tunnel plus concurrent H3 streaming workload, measured low-level `quiche`/`tokio-quiche` raw tunnel echo rows, measured n=30 low-level `quiche`/`tokio-quiche` close/FIN rows, measured n=30 low-level `quiche`/`tokio-quiche` slow-consumer mixed rows, and a persisted n=100 echo artifact with a passing dedicated echo tunnel superiority gate. A full tunnel-suite superiority claim is not made yet: the mixed rows are now present, but the current gate still covers echo only and the mixed workload needs optimization/gate definition before claiming suite-wide superiority.
...
Specter status:
- Fixed: H3/RFC9220 tunnel outbound sends now acquire byte permits and release them per emitted wire chunk, and inbound tunnel delivery uses an unbounded driver-to-handle channel guarded by receive-side byte permits.
...
- RFC9220/WebSocket-over-H3 now has n=100 echo/close/mixed samples for Specter, n=100 low-level `quiche`/`tokio-quiche` echo comparator rows, n=30 low-level `quiche`/`tokio-quiche` close/FIN comparator rows, n=30 low-level `quiche`/`tokio-quiche` slow-consumer mixed comparator rows, and a dedicated RFC9220 echo tunnel gate; full-suite superiority remains open because gate expansion and mixed-workload optimization are still unresolved.
...
- Specter RFC9220 local tunnel throughput/latency rows, n=100 p99-scale samples, low-level `quiche`/`tokio-quiche` raw tunnel echo comparator rows, low-level `quiche`/`tokio-quiche` close/FIN comparator rows, low-level `quiche`/`tokio-quiche` slow-consumer mixed comparator rows, and the dedicated echo tunnel superiority gate are no longer pending; unsupported higher-level client capability rows remain explicit capability gaps, and full-suite superiority remains unclaimed.
...
- RFC9220 tunnel backpressure is no longer item-count-only: public sends acquire a per-tunnel outbound byte budget via `H3TunnelCredit`'s semaphore (`DEFAULT_H3_TUNNEL_OUTBOUND_BYTE_BUDGET = 256 KiB`, `MIN_H3_TUNNEL_OUTBOUND_BYTE_BUDGET = 1 KiB`, configurable through `H3Client::with_tunnel_outbound_byte_budget`), the driver releases permits proportionally on transmit via `DriverPendingTunnelOutbound`, and inbound driver-to-handle delivery uses an unbounded channel guarded by receive-byte permits from `H3TransportConfig::tunnel_inbound_byte_budget`. Tunnel reads release inbound byte permits and feed encoded DATA-frame byte credit into MAX_STREAM_DATA. Coverage: `cargo test --lib tunnel_inbound --no-default-features` and `cargo test --lib recv_event_releases_encoded_data_frame_credit --no-default-features`.
...
- Path migration primitives are no longer just packet/token handling: the new `src/transport/h3/path.rs` adds RFC9000 § 5.1 `QuicConnectionIdInventory` (locally issued / peer-issued CIDs by sequence number, `active_connection_id_limit` enforcement, NEW_CONNECTION_ID / RETIRE_CONNECTION_ID processing with retire_prior_to handling), RFC9000 § 8.1 `QuicAntiAmplificationLimit` (per-path 3x send-budget accounting, validated promotion removes the cap), and RFC9000 § 9 `QuicPathSet`/`QuicPath` (per-address Probing/Validating/Validated/Primary/Abandoned state with PATH_CHALLENGE token tracking and primary path demotion on promotion). Client PATH_RESPONSE handling now promotes the validated peer-issued CID for future 1-RTT packets. Driver-level wiring (NEW_CONNECTION_ID emission after handshake, accepting/promoting a migrated peer address in the driver loop, outbound-builder anti-amplification gate, server migration lifecycle) remains the P1 item.

## Native QUIC/H3 protocol gaps

### P0

1. **RFC9220 full tunnel-suite superiority proof:** Specter local tunnel echo, close/FIN, slow-consumer mixed rows, low-level `quiche`/`tokio-quiche` raw tunnel echo rows, low-level `quiche`/`tokio-quiche` close/FIN rows, low-level `quiche`/`tokio-quiche` slow-consumer mixed rows, and a dedicated echo tunnel superiority gate are persisted; full-suite superiority still needs explicit gate expansion plus mixed-workload optimization/evidence before it can be claimed.

### Recently closed P1

1. **RFC-grade CONNECTION_CLOSE drain + RTT/PTO wiring** (RFC9000 § 10.2 / § 10.2.1, RFC9002 § 5.1–5.3 / § 6.2.1): per-direction `QuicCloseState { Open | Closing | Draining }` plus `QuicLossDetector` RTT estimator (`latest_rtt`, `smoothed_rtt`, `rttvar`, `min_rtt`, `peer_ack_delay_exponent`, `max_ack_delay`, `current_pto()`, `close_window() = 3 * PTO`), `on_ack_frame_at` RTT sampling with ack-delay subtraction and min_rtt floor, and driver `run_close_window` rate-limited CONNECTION_CLOSE replay (min interval = current PTO, packet threshold = 1). Files: `src/transport/h3/quic.rs`, `src/transport/h3/handshake.rs`, `src/transport/h3/native_driver.rs`. Tests: 8 unit in `quic.rs::close_state_tests` and 4 integration in `tests/h3_native_handshake.rs`. CHANGELOG: `[Unreleased] / Added`.
2. **RFC9001 § 6 1-RTT key update** (§ 6 / § 6.1 / § 6.2 / § 6.5): `derive_next_application_secret` (HKDF-Expand-Label `"quic ku"`) and `derive_next_packet_key_material` (rotates packet key + IV, preserves HP key), `PreviousKeys` + `PREVIOUS_KEY_WINDOW = 3s` reorder window, `OneRttKeyUpdate` in-progress tracking, `try_open_one_rtt_packet` trial decrypt with phase verification, per-direction next/previous keys + `write_key_phase` / `read_key_phase`, and public `force_key_update()` / `write_key_phase()` / `read_key_phase()` / `key_update_in_progress()` on both client and server handshakes. Files: `src/transport/h3/quic.rs`, `src/transport/h3/handshake.rs`. Tests: 2 in `tests/h3_native_quic.rs` (HKDF regression vector + HP-key parity), 3 in `tests/h3_native_handshake.rs` (reorder, double-update error, ACK confirms update). CHANGELOG: `[Unreleased] / Added`.
3. **TLS session resumption + QUIC 0-RTT** (RFC 8446 § 4.6.1, RFC9001 § 4.6 / § 9.2): `NativeH3HandshakeStatus { None | Resumed | EarlyAccepted | EarlyRejected }` + `early_data_reason()`, `server_with_ticket_keys` / `server_with_connection_ids_and_ticket_keys`, `SSL_set_early_data_enabled` gated to explicit opt-in, server `SSL_set_quic_early_data_context`, new `NativeH3SessionCache` keyed by `(SNI, ALPN list, verify_peer, Option<fingerprint_pin>)` with TTL / capacity / expiry-on-lookup / oldest eviction, `NativeQuicTlsSession::client_with_zero_rtt_offer(sni, fp, tls_fp, verify, Option<session_ticket_der>, early_data)` per-call opt-in, and `NativeH3TlsCapabilities::current()` now reporting Supported / Supported. Files: `src/transport/h3/tls.rs`, `src/transport/h3/session_cache.rs`, `src/transport/h3/mod.rs`, `src/transport/h3/handshake.rs`, `src/fingerprint/tls.rs`. Tests: 5 round-trip in `tests/h3_native_tls_resumption.rs`, 7 unit in `session_cache.rs::tests`. CHANGELOG: `[Unreleased] / Added`. Follow-up: `session_cache::tests::expired_entries_are_evicted_on_lookup` is timing-flaky (50ms TTL / 80ms sleep) and should be hardened, not treated as a correctness gap.
4. **Byte-bounded outbound RFC9220 tunnel backpressure**: `H3TunnelCredit` switched to `tokio::sync::Semaphore` + `send_budget`, `H3Tunnel::send_bytes` calls `acquire_many(min(bytes.len(), budget))` before forwarding through an unbounded mpsc, `close_send` stays fin-only, driver-side `DriverPendingTunnelOutbound { bytes, fin, credit_released, acquired_credit }` releases permits proportionally on transmit (release-on-transmit), and new config knobs `DEFAULT_H3_TUNNEL_OUTBOUND_BYTE_BUDGET = 256 KiB` / `MIN_H3_TUNNEL_OUTBOUND_BYTE_BUDGET = 1 KiB` / `H3TransportConfig::tunnel_outbound_byte_budget` (clamped in `normalized()`) / `H3Client::with_tunnel_outbound_byte_budget`. Files: `src/transport/h3/mod.rs`, `src/transport/h3/tunnel.rs`, `src/transport/h3/native_driver.rs`. Tests: 5 new `tunnel.rs` unit tests, existing rfc9220_* integration tests still 12/12 green. CHANGELOG: `[Unreleased] / Changed`.
5. **RFC9000 § 4 absolute-value H3 receive flow control** (§ 4.1 / § 4.2 / § 19.9 / § 19.10): `QuicReceiveFlowControl` rewritten to advertise `initial + sum(bytes_consumed_by_application)`, new `record_stream_consumed(stream_id, len)` / `release_stream(stream_id)` plus public `NativeQuicHandshake::record_client_stream_consumed` / `release_client_stream` / `record_server_stream_consumed` / `release_server_stream`, driver `apply_released_body_credits` / `apply_released_tunnel_credits` thread every per-stream release through `record_client_stream_consumed` (return type now `Result<usize>`), `observe_stream_frame` becomes pure enforcement, formula clamped at `max_stream_window`, emission gating unchanged at `max(1, initial / 2)`. Files: `src/transport/h3/handshake.rs`, `src/transport/h3/native_driver.rs`, plus collateral wiring in `tests/helpers/mock_h3_server.rs` and `benches/native_h3_vs_rust_clients/src/main.rs`. Tests: 5 unit in `handshake.rs::receive_flow_control_tests` and existing receive-threshold integration tests rewritten against the new API. CHANGELOG: `[Unreleased] / Fixed`.

### P1

1. **Full path migration driver integration:** client PATH_CHALLENGE packetization, matching PATH_RESPONSE validation, required transport-parameter CIDs, 1-RTT CID routing, and post-validation client destination-CID promotion exist; the new `src/transport/h3/path.rs` module now provides primitive coverage for RFC9000 § 5.1 connection-ID inventory (locally issued and peer-issued CIDs with sequence numbers, `active_connection_id_limit` enforcement, NEW_CONNECTION_ID / RETIRE_CONNECTION_ID processing), RFC9000 § 8.1 per-path anti-amplification 3x send-budget accounting, and RFC9000 § 9 per-address path state (Primary/Probing/Validating/Validated/Abandoned). Driver-level integration (issuing NEW_CONNECTION_ID after handshake completion, accepting/promoting a migrated peer address in `native_driver`, gating outbound packet builders on the anti-amplification budget, server-side migration lifecycle) is the remaining work.
...
5. **Expand RFC9220 benchmark rows**
   - Raw byte tunnel: echo RTT, close/FIN latency, and slow-consumer mixed workload are now measured for Specter locally at n=100; low-level `quiche`/`tokio-quiche` echo rows are n=100, close/FIN rows are n=30, and slow-consumer mixed rows are n=30. The next gap is full-suite gate expansion and improving/proving the mixed workload before claiming suite-wide superiority.
...
6. **Close native QUIC production gaps**
   - Continue from landed PTO send-time tracking, client/server CRYPTO PTO retransmission, client application PTO, server application recovery core, mock/same-fixture wake integration, RFC9000 § 10.2/§ 10.2.1 RFC-grade close drain with RFC9002 § 6.2.1 `close_window = 3 * PTO` and rate-limited replay, RFC9001 § 6 1-RTT key update with HP-key preservation and 3s reorder window, RFC 8446 § 4.6.1 / RFC9001 § 4.6/§ 9.2 TLS session resumption plus opt-in QUIC 0-RTT, byte-bounded RFC9220 outbound tunnel backpressure, and RFC9000 § 4 absolute-value MAX_DATA / MAX_STREAM_DATA emission toward broader recovery soak/backoff validation.
   - Keep Retry/version negotiation, CID routing, close-drain replay, key update, TLS resumption + 0-RTT, byte-bounded RFC9220 tunnel backpressure, and absolute-value flow control under regression coverage while finishing path migration.
...
- Local RFC9220 H3 combined n=100 echo proof plus n=30 close and mixed comparators: `docs/benchmarks/native-h3-vs-rust-clients/2026-05-25-rfc9220-n100-plus-close-and-mixed-comparators.json`
