...
- ACK_ECN is no longer just parse/round-trip coverage: native loss detection validates counters, tracks CE growth, disables ECN on invalid counters, and reduces congestion window on CE growth; only outbound socket marking and probing policy remain open.
...
1. **0-RTT policy:** certificate compression, session-ticket capture/install helpers, `NativeH3SessionCache`, H3Client/session-cache wiring, TLS session replay, and 0-RTT early-data context setup exist, but native H3 still lacks safe end-to-end 0-RTT request send/replay policy.

### P2

1. **ACK_ECN / ECN plumbing:** ACK_ECN frame encode/decode, counter validation, invalid-counter ECN disablement, and CE-driven congestion response exist; ECN socket marking and PMTU/path probing policy are still missing.
...
6. **Close native QUIC production gaps**
   ...
   - Finish ECN socket marking, PMTU/path probing, and browser capture parity after the recovery/state-machine core is stable.
