...
- **Native QUIC production readiness:** still not production-complete; PTO send-time tracking, ACK-driven RTT/PTO estimator updates, client Initial/Handshake plus server Initial/Handshake CRYPTO PTO retransmission, client application-space PTO timer/retransmit, server application ACK-driven recovery plus PTO STREAM retransmit core, event-level peer-close draining, bounded client CONNECTION_CLOSE replay, 1-RTT key-update handling, ACK_ECN frame/counter validation, Retry/VN client-handshake handling, client PATH_CHALLENGE/PATH_RESPONSE token lifecycle, and H3Client-level native session-cache wiring exist, but full packet-space recovery/backoff, server-side fixture/driver application loss-detection wake integration, RFC-grade close-drain timing, ECN marking/congestion response, per-address path migration, and 0-RTT replay policy remain gaps.
...
- RFC9220/WebSocket-over-H3 p99 still needs n>=100 to be statistically meaningful, third-party close/FIN and slow-consumer comparator rows remain open, and any tunnel superiority claim needs a dedicated RFC9220 gate rather than relying on the H3 HTTP gate.
...
- TLS extension-order behavior is no longer an evidence-free gap: native H3 honors deterministic-vs-browser-permuted ClientHello generation policy. Session-ticket capture/replay, `NativeH3SessionCache`, H3Client cache injection/access, connection-establishment session lookup/eviction fallback, driver-side ticket drain, and 0-RTT early-data context helpers also exist; the remaining high-level gap is safe end-to-end 0-RTT request send/replay policy.
...
- Server application-space recovery core is no longer absent: native H3 records server response STREAM packets in `RecoveryState`, retires them on client ACKs, carries recovery-detected losses into retransmit selection, and can PTO-retransmit unacked server STREAM packets with fresh packet numbers.
...
4. **Fingerprinting capture gaps:** TLS certificate compression, extension-order behavior control, session-ticket helpers, H3Client-level `NativeH3SessionCache` wiring, and raw ordered transport-parameter encode are in place; capture-derived raw transport-parameter presets, explicit extension-list ordering beyond BoringSSL permutation policy, dynamic connection-ID placeholders inside raw lists, and full H3 0-RTT replay-policy integration remain open.
...
6. **Close native QUIC production gaps**
   - Continue from landed PTO send-time tracking, client/server CRYPTO PTO retransmission, client application PTO, and server application recovery core toward server-side fixture/driver application loss-detection wake integration, full packet-space recovery/PTO, and production backoff policy.
