...
- **Native QUIC production readiness:** still not production-complete; PTO send-time tracking, ACK-driven RTT/PTO estimator updates, client Initial/Handshake plus server Initial/Handshake CRYPTO PTO retransmission, client application-space PTO timer/retransmit, server application ACK-driven recovery plus mock/same-fixture PTO STREAM retransmit wake, event-level peer-close draining, bounded client CONNECTION_CLOSE replay, 1-RTT key-update handling, ACK_ECN frame/counter validation, Retry/VN client-handshake handling, client PATH_CHALLENGE/PATH_RESPONSE token lifecycle, and H3Client-level native session-cache wiring exist, but broader PTO/backoff and persistent-congestion hardening, RFC-grade close-drain timing, ECN marking/congestion response, per-address path migration, and 0-RTT replay policy remain gaps.
...
6. **Close native QUIC production gaps**
   - Continue from landed PTO send-time tracking, client/server CRYPTO PTO retransmission, client application PTO, server application recovery core, and mock/same-fixture wake integration toward broader PTO/backoff hardening and RFC-grade close-drain behavior.
