...
- RFC9220 tunnel backpressure is no longer item-count-only on the public-send / outbound path: public sends acquire a per-tunnel outbound byte budget via `H3TunnelCredit`'s semaphore (`DEFAULT_H3_TUNNEL_OUTBOUND_BYTE_BUDGET = 256 KiB`, `MIN_H3_TUNNEL_OUTBOUND_BYTE_BUDGET = 1 KiB`, configurable through `H3Client::with_tunnel_outbound_byte_budget`), the driver releases permits proportionally on transmit via `DriverPendingTunnelOutbound`, and inbound delivery reserves receive byte credit before driver-to-handle delivery and feeds MAX_STREAM_DATA from public reads. The driver-to-handle inbound channel itself is still item-bounded (`NativeDriverTunnelState`'s 32-slot mpsc plus `pending_inbound` overflow and `is_inbound_backpressured` heuristic); see the inbound-channel P1 item below. Slow-consumer mixed RFC9220 coverage remains green after the outbound change.
...
2. **Inbound RFC9220 tunnel channel is still item-bounded:** `src/transport/h3/native_driver.rs::NativeDriverTunnelState` uses `mpsc::channel(32)` for `inbound_tx` / `inbound_rx`, plus a `pending_inbound: VecDeque` overflow path and an `is_inbound_backpressured` heuristic. `H3TunnelCredit::released_recv_bytes` already feeds byte-credit accounting into MAX_STREAM_DATA, but the driver-to-handle channel itself does not yet bound on bytes; tiny chunks can still consume 32 inbound slots while large chunks bypass the byte budget on the way to the handle. Remaining work is to make the driver-to-handle inbound path byte-bounded (matching the outbound side's semaphore + unbounded mpsc shape) and verify it via the existing rfc9220_* slow-consumer and mixed integration tests.
...
6. **Close native QUIC production gaps**
   ...
   - Keep Retry/version negotiation, CID routing, close-drain replay, key update, TLS resumption + 0-RTT, outbound tunnel byte budget, and absolute-value flow control under regression coverage while finishing path migration and byte-bounding the inbound RFC9220 tunnel channel.
