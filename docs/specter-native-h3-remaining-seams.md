...
- Added byte-bounded RFC9220 tunnel backpressure on the public-send / outbound path: public sends acquire per-tunnel outbound byte permits and the native driver releases them per emitted DATA chunk, while inbound driver-to-handle delivery reserves receive byte permits and feeds MAX_STREAM_DATA from public reads (the driver-to-handle channel itself is still item-bounded; see remaining gaps).
...
- RFC9220 tunnel backpressure is byte-bounded at the public-send / outbound path: public sends block on outbound byte permits via `H3TunnelCredit`'s semaphore (`DEFAULT_H3_TUNNEL_OUTBOUND_BYTE_BUDGET = 256 KiB`, configurable through `H3Client::with_tunnel_outbound_byte_budget`), the driver releases permits proportionally on transmit via `DriverPendingTunnelOutbound`, and inbound DATA delivery reserves receive byte credit before queuing into the still-item-bounded driver-to-handle channel and feeds MAX_STREAM_DATA from public reads. Slow-consumer mixed RFC9220 coverage remains green after the outbound change, and focused inbound tests cover tiny chunks versus oversized chunks. The driver-to-handle inbound channel byte-bounding remains in `Remaining gaps`.
