...
- Native QUIC frame codec now round-trips RFC9000 ACK_ECN frames (`0x03`), validates ACK_ECN counters, records CE growth, applies ACK_ECN ranges like ordinary ACK ranges, feeds CE growth into congestion response, generates ACK_ECN counters from observed ECT(0)/ECT(1)/CE receive marks, and can apply fingerprint-controlled outbound ECT(0)/ECT(1) socket marking.
...
- Native H3 TLS now advertises certificate compression from the TLS fingerprint, controls deterministic-vs-browser-permuted extension behavior, emits raw ordered QUIC transport parameters with dynamic connection-ID placeholders when configured, wires session-ticket capture/replay through `NativeH3SessionCache`, H3Client cache injection/access, H3 connection establishment, driver-side ticket drain, and H3Handle/H3Client-level status reporting, and proves ordinary resumption suppresses 0-RTT CRYPTO unless policy opts in; safe end-to-end 0-RTT request replay policy remains open.
...
The low-level `quiche_direct_rfc9220_tunnel` and `tokio_quiche_rfc9220_tunnel` adapters now have measured n=100 echo rows. `h3_quinn_rfc9220_tunnel`, `reqwest_h3_rfc9220_tunnel`, `tokio_tungstenite_rfc9220`, and `reqwest_rfc9220` remain unsupported capability-audit rows, not throughput comparators. Third-party close/FIN and mixed slow-consumer tunnel comparator rows remain open.
...
- Added ACK_ECN frame encode/decode support, counter validation, CE growth tracking, ACK_ECN range handling, CE-driven congestion response, ACK_ECN generation from observed receive marks, and fingerprint-controlled outbound ECN UDP socket marking in the native QUIC path.
...
- Native QUIC ACK_ECN frame encode/decode, counter validation, CE growth tracking, loss-detector ACK range handling, CE-driven congestion response, ACK_ECN generation from observed ECT(0)/ECT(1)/CE receive marks, and fingerprint-controlled outbound ECT(0)/ECT(1) UDP socket marking are implemented; remaining ECN work is socket-level receive ECN reporting into the tracker and PMTU/path probing policy.
...
- RFC9220/WebSocket-over-H3 has Specter-native same-fixture echo, close/FIN, slow-consumer mixed rows, and low-level `quiche`/`tokio-quiche` raw tunnel echo comparator rows at n=100; remaining proof work is third-party close/FIN plus slow-consumer comparator rows and any dedicated tunnel superiority gate/claim.
- TLS certificate compression, deterministic-vs-browser-permuted extension behavior, raw ordered QUIC transport parameters with dynamic connection-ID placeholders, session-ticket helpers, `NativeH3SessionCache`, H3Client cache wiring, connection-level session replay, driver-side ticket drain, 0-RTT early-data context setup, and H3Handle/H3Client native handshake-status reporting are wired for native H3; remaining fingerprint work is explicit extension-list ordering, 0-RTT replay-policy integration, and capture presets.
- TLS resumption is now plumbed from H3Client through `SSL_SESSION` replay and ticket storage; ordinary session replay now strips early-data capability unless request policy opts in, and TLS-level 0-RTT accept/reject status plus reason codes are observable through the connection handle and H3Client. The remaining 0-RTT gap is anti-replay request policy and transport send integration, not ambiguity, missing cache wiring, or missing status propagation.
...
- Native QUIC still needs broader recovery soak/backoff validation, socket-level receive ECN reporting into ACK_ECN mark observation, PMTU/path probing policy, and path migration/validation beyond the client token lifecycle.
...
- RFC9220/WebSocket-over-H3 still lacks third-party close/FIN plus slow-consumer comparator rows and a dedicated tunnel superiority gate/claim, even though Specter echo/close/mixed and low-level `quiche`/`tokio-quiche` raw tunnel echo comparator adapters now have n=100 rows.
- TLS/H3 fingerprint gaps remain: explicit extension-list ordering beyond BoringSSL permutation policy, 0-RTT request send/replay policy, and capture-derived raw transport-parameter presets.
