...
- Native QUIC frame codec now round-trips RFC9000 ACK_ECN frames (`0x03`), validates ACK_ECN counters, records CE growth, applies ACK_ECN ranges like ordinary ACK ranges, feeds CE growth into congestion response, and can apply fingerprint-controlled outbound ECT(0)/ECT(1) socket marking.
...
- Added ACK_ECN frame encode/decode support, counter validation, CE growth tracking, ACK_ECN range handling, CE-driven congestion response, and fingerprint-controlled outbound ECN UDP socket marking in the native QUIC path.
...
- Native QUIC ACK_ECN frame encode/decode, counter validation, CE growth tracking, loss-detector ACK range handling, CE-driven congestion response, and fingerprint-controlled outbound ECT(0)/ECT(1) UDP socket marking are implemented; remaining ECN work is receive-side ECN reporting, ACK_ECN generation from received marks, and PMTU/path probing policy.
...
- Native QUIC still needs broader recovery soak/backoff validation, receive-side ECN reporting plus ACK_ECN generation from received marks, PMTU/path probing policy, and path migration/validation beyond the client token lifecycle.
