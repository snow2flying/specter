...
- Native QUIC still needs full packet-space RFC9002 production recovery with server-side fixture/driver application loss-detection wake integration, broader PTO/backoff policy, RFC-grade close-drain timing beyond the bounded client replay window, ECN socket marking plus CE-driven congestion response, and path migration/validation beyond the client token lifecycle.
