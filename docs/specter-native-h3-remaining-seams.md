...
- RFC9220 mixed comparator proof is `docs/benchmarks/native-h3-vs-rust-clients/2026-05-25-rfc9220-n100-plus-close-and-mixed-comparators.json`. It merges the n=100 echo-gate proof with n=30 measured low-level `quiche` and `tokio-quiche` close/FIN plus slow-consumer mixed rows, keeps the dedicated echo gate passing, and emits no `fixture_events`; it does not claim full-suite superiority.
...
- Added byte-bounded RFC9220 tunnel backpressure on both directions: public sends acquire per-tunnel outbound byte permits and the native driver releases them per emitted DATA chunk, while inbound driver-to-handle delivery uses an unbounded channel guarded by receive-byte permits and feeds MAX_STREAM_DATA from public reads.
...
- Added native QUIC path migration primitives in `src/transport/h3/path.rs`: `QuicConnectionIdInventory` (RFC9000 § 5.1 locally issued / peer-issued CID tracking with sequence numbers, `active_connection_id_limit` enforcement per RFC9000 § 18.2, NEW_CONNECTION_ID validation per § 19.15 with retire_prior_to handling, RETIRE_CONNECTION_ID processing per § 19.16, queued outbound retires, peer-active promotion gate); `QuicAntiAmplificationLimit` (RFC9000 § 8.1 per-path 3x send-budget accounting with validated-state cap removal); and `QuicPathSet`/`QuicPath`/`QuicPathState` (RFC9000 § 9 per-address path state with Probing/Validating/Validated/Primary/Abandoned transitions, PATH_CHALLENGE token tracking, validated-promotion-removes-cap, and demote-previous-primary-on-promotion semantics). Client PATH_RESPONSE handling now promotes the validated peer-issued CID for future 1-RTT packets. Driver integration (NEW_CONNECTION_ID emission after handshake, accepting/promoting a migrated peer address in `native_driver`, anti-amplification gate on outbound builders, server migration lifecycle) is the remaining P1 driver-integration step.
- Added RFC-grade CONNECTION_CLOSE drain + RTT/PTO wiring (RFC9000 § 10.2 / § 10.2.1, RFC9002 § 5.1–5.3 / § 6.2.1): a per-direction `QuicCloseState { Open | Closing | Draining }` embedded on both `NativeQuicHandshake` and `NativeQuicServerHandshake`, a `QuicLossDetector` RTT estimator (`latest_rtt`, `smoothed_rtt`, `rttvar`, `min_rtt`, `peer_ack_delay_exponent`, `max_ack_delay`, `current_pto()`, `close_window() = 3 * PTO`) with `on_ack_frame_at` taking RTT samples on the largest newly-acked packet (ack-delay subtraction, min_rtt floor), accessors `close_state` / `*_close_window` / `*_is_close_window_expired` / `*_should_replay_connection_close` / `*_mark_connection_close_replayed` / `*_observe_inbound_packet_for_close` / `*_application_pto`, `build_*_connection_close_packet` entering closing, `open_*` parsers early-returning only when draining (closing -> draining MAY-optimisation allowed), and a driver `run_close_window` replacing the previous fixed `drain_connection_close` loop with rate-limited CONNECTION_CLOSE replay (min interval = current PTO, packet threshold = 1). Production files: `src/transport/h3/quic.rs`, `src/transport/h3/handshake.rs`, `src/transport/h3/native_driver.rs`. Test files added: 8 unit in `src/transport/h3/quic.rs::close_state_tests` and 4 integration in `tests/h3_native_handshake.rs` (local closing entry, rate-limited replay, peer-close supersession, server-side closing). CHANGELOG: `[Unreleased] / Added`.
- Added RFC9001 § 6 1-RTT key update (§ 6 / § 6.1 / § 6.2 / § 6.5): `derive_next_application_secret` (HKDF-Expand-Label `"quic ku"`) and `derive_next_packet_key_material` (rotates packet key + IV, preserves HP key per § 6.1), `PreviousKeys` + `PREVIOUS_KEY_WINDOW = 3s` (§ 6.2 reorder window), `OneRttKeyUpdate` for § 6.5 in-progress tracking, `try_open_one_rtt_packet` doing current / previous / next trial decrypt with phase verification, per-direction next/previous keys plus `write_key_phase` / `read_key_phase` on both handshakes, six 1-RTT send sites stamping the local write phase, both receive sites routing through trial-decrypt and mirroring the local rotation, ACK processing clearing in-progress via the anchor, and public `force_key_update()` / `write_key_phase()` / `read_key_phase()` / `key_update_in_progress()` on both client and server handshakes. Production files: `src/transport/h3/quic.rs`, `src/transport/h3/handshake.rs`. Test files added: 2 in `tests/h3_native_quic.rs` (HKDF regression vector + HP-key parity) and 3 in `tests/h3_native_handshake.rs` (reorder, double-update error, ACK confirms update). CHANGELOG: `[Unreleased] / Added`.
- Added TLS session resumption plus QUIC 0-RTT (RFC 8446 § 4.6.1, RFC9001 § 4.6 / § 9.2): `NativeH3HandshakeStatus { None | Resumed | EarlyAccepted | EarlyRejected }` with `handshake_status()` / `early_data_reason()` accessors, `NATIVE_H3_TICKET_KEY_LEN`, `server_with_ticket_keys` and `server_with_connection_ids_and_ticket_keys` server builders, `SSL_set_early_data_enabled` gated to explicit opt-in, server `SSL_set_quic_early_data_context`, a new `NativeH3SessionCache` keyed by `(SNI, ALPN list, verify_peer, Option<fingerprint_pin>)` with TTL / capacity / expiry-on-lookup / oldest eviction, `NativeQuicTlsSession::client_with_zero_rtt_offer(sni, fp, tls_fp, verify, Option<session_ticket_der>, early_data)` per-call opt-in (no default), handshake-status / early-data-reason mirrored on both handshakes, and `NativeH3TlsCapabilities::current()` now reporting Supported / Supported (was Unsupported / Unsupported). Production files: `src/transport/h3/tls.rs`, `src/transport/h3/session_cache.rs`, `src/transport/h3/mod.rs`, `src/transport/h3/handshake.rs`, `src/fingerprint/tls.rs`. Test files added: 5 round-trip in `tests/h3_native_tls_resumption.rs` (ticket capture, replay -> Resumed, 0-RTT -> EarlyAccepted or clean EarlyRejected, fingerprint mismatch never EarlyAccepted, extension-order policy switch moves cache row) and 7 unit in `src/transport/h3/session_cache.rs::tests`. CHANGELOG: `[Unreleased] / Added`.
- Added byte-bounded outbound RFC9220 tunnel backpressure: `H3TunnelCredit` switched to a `tokio::sync::Semaphore` + `send_budget`, `H3Tunnel::send_bytes` calls `acquire_many(min(bytes.len(), budget))` before forwarding through an unbounded mpsc, `close_send` stays fin-only and skips the semaphore, driver-side `DriverPendingTunnelOutbound { bytes, fin, credit_released, acquired_credit }` releases permits proportionally on transmit (release-on-transmit, not release-on-consume), and new config knobs `DEFAULT_H3_TUNNEL_OUTBOUND_BYTE_BUDGET = 256 KiB` / `MIN_H3_TUNNEL_OUTBOUND_BYTE_BUDGET = 1 KiB` / `H3TransportConfig::tunnel_outbound_byte_budget` (clamped in `normalized()`) / `H3Client::with_tunnel_outbound_byte_budget` builder + getter. Production files: `src/transport/h3/mod.rs`, `src/transport/h3/tunnel.rs`, `src/transport/h3/native_driver.rs`. Test files added: 5 new unit tests in `src/transport/h3/tunnel.rs`; existing rfc9220_* integration tests still 12/12 green (3+3+2+2+2). CHANGELOG: `[Unreleased] / Changed`.
- Added RFC9000 § 4 absolute-value H3 receive flow control (§ 4.1 / § 4.2 / § 19.9 / § 19.10): rewrote `QuicReceiveFlowControl` to advertise `initial + sum(bytes_consumed_by_application)` instead of receive-threshold deltas, added `record_stream_consumed(stream_id, len)` / `release_stream(stream_id)` plus public `NativeQuicHandshake::record_client_stream_consumed` / `release_client_stream` / `record_server_stream_consumed` / `release_server_stream`, plumbed driver `apply_released_body_credits` / `apply_released_tunnel_credits` through `record_client_stream_consumed` for every per-stream release (return type now `Result<usize>`, 3 call sites updated) and called `release_client_stream` on close, turned `observe_stream_frame` into pure enforcement, clamped per-stream at `max_stream_window`, and kept emission gating at `max(1, initial / 2)`. Production files: `src/transport/h3/handshake.rs`, `src/transport/h3/native_driver.rs`, plus collateral wiring in `tests/helpers/mock_h3_server.rs` and `benches/native_h3_vs_rust_clients/src/main.rs` (server-side `record_server_stream_consumed`). Test files added: 5 unit in `src/transport/h3/handshake.rs::receive_flow_control_tests`; existing 2 receive-threshold integration tests already rewritten by a sibling against the new API. CHANGELOG: `[Unreleased] / Fixed`.
...
- RFC9220/WebSocket-over-H3 has Specter-native same-fixture echo, close/FIN, slow-consumer mixed rows, low-level `quiche`/`tokio-quiche` raw tunnel echo comparator rows at n=100, low-level `quiche`/`tokio-quiche` close/FIN comparator rows at n=30, low-level `quiche`/`tokio-quiche` slow-consumer mixed comparator rows at n=30, and a passing dedicated echo tunnel superiority gate; remaining proof work is full-suite gate expansion plus mixed-workload optimization/evidence before making a suite-wide superiority claim.
...
- RFC9220 tunnel backpressure is byte-bounded in both directions: public sends block on outbound byte permits via `H3TunnelCredit`'s semaphore (`DEFAULT_H3_TUNNEL_OUTBOUND_BYTE_BUDGET = 256 KiB`, configurable through `H3Client::with_tunnel_outbound_byte_budget`), the driver releases permits proportionally on transmit via `DriverPendingTunnelOutbound`, and inbound DATA delivery uses an unbounded driver-to-handle channel guarded by `H3TransportConfig::tunnel_inbound_byte_budget` receive permits. Slow-consumer mixed RFC9220 coverage remains green after the backpressure changes, and focused inbound tests cover tiny chunks versus oversized chunks (`cargo test --lib tunnel_inbound --no-default-features`) plus encoded DATA-frame credit release (`cargo test --lib recv_event_releases_encoded_data_frame_credit --no-default-features`).
...
- Native QUIC still needs broader recovery soak/backoff validation and full per-address path migration driver integration: issuing NEW_CONNECTION_ID after handshake, accepting/promoting a migrated peer address in `native_driver`, gating outbound packet builders on the anti-amplification budget, and server-side migration lifecycle. Basic PATH_CHALLENGE token handling, CID inventory primitives, anti-amplification primitives, 1-RTT CID routing, and post-validation client destination-CID promotion are no longer active gaps.
