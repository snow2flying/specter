...
- Native QUIC now has send-time tracking, ACK-driven RTT/PTO estimator updates, client Initial/Handshake plus server Initial/Handshake CRYPTO PTO retransmission, client application-space driver PTO timer/retransmit, server application ACK-driven recovery state plus PTO STREAM retransmit helpers, event-level peer close draining, Retry/VN client-handshake handling, and client PATH_CHALLENGE/PATH_RESPONSE token lifecycle coverage.
...
- Added QUIC send-time tracking, client Initial/Handshake CRYPTO PTO retransmission, and server Initial/Handshake CRYPTO PTO retransmission while preserving CRYPTO offsets and fresh packet numbers; full packet-space recovery remains pending.
...
- QUIC send-time tracking, ACK-driven RTT/PTO estimator updates, client Initial/Handshake CRYPTO PTO retransmission, server Initial/Handshake CRYPTO PTO retransmission, client application-space PTO timer/retransmit, server application ACK-driven recovery and PTO STREAM retransmit core, and event-level peer close draining are implemented; remaining recovery work is full production loss recovery, server-side fixture/driver application loss-detection wake integration, broader packet-space/backoff policy, and close-drain timer/retransmit behavior.
...
- RFC9002 packet-space recovery/PTO is now complete enough for production loss handling: `src/transport/h3/recovery.rs` (new module) implements a per-space `RttEstimator` (`smoothed_rtt`/`rttvar`/`min_rtt` with `ack_delay` subtraction), per-space `PacketSpaceRecovery` (sent_packets, largest_acked, loss_time, time_of_last_ack_eliciting_packet), NewReno-minimum `CongestionController` (cwnd, ssthresh, bytes_in_flight, congestion-recovery epoch), and a `RecoveryState` aggregate driving RFC9002 § 6 packet/time loss thresholds, `pto_time_and_space` per-space PTO with anti-deadlock fallback, `pto_count` backoff that doubles per timeout and resets on ack-eliciting ACKs, persistent-congestion detection, and `discard_space` for Initial/Handshake epoch teardown. The client `NativeQuicHandshake` records Initial sends (`record_client_initial_sent_at`), retransmits Initial CRYPTO on PTO (`retransmit_pto_client_initial_crypto_packets`) with preserved offsets, wires `recovery.on_packet_sent`/`on_ack_received` for Initial/Handshake/Application send and ACK paths, and exposes `recovery()`, `loss_detection_timer()`, `application_pto()`, and `on_loss_detection_timeout()` for the H3 driver; the server-side `NativeQuicServerHandshake` exposes the same surface plus `retransmit_pto_server_application_stream_packets`. Remaining recovery work is server-side fixture/driver application loss-detection wake integration tied into the H3 server driver, RFC-grade close-drain timing tied to PTO, and ECN socket marking plus CE-driven congestion response.

## Remaining gaps

- Native QUIC still needs server-side fixture/driver application loss-detection wake integration, RFC-grade close-drain timing beyond the bounded client replay window, ECN socket marking plus CE-driven congestion response, and path migration/validation beyond the client token lifecycle.
