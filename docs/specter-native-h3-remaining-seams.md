...
- Native QUIC still needs broader recovery soak/backoff validation and full per-address path migration driver integration: issuing NEW_CONNECTION_ID after handshake, gating outbound packet builders on the anti-amplification budget, and server-side migration lifecycle. Basic PATH_CHALLENGE token handling, CID inventory primitives, anti-amplification primitives, 1-RTT CID routing, post-validation client destination-CID promotion, and native-driver migrated peer-address promotion are no longer active gaps.
