...
- Added native QUIC path migration primitives in `src/transport/h3/path.rs`: `QuicConnectionIdInventory` (RFC9000 § 5.1 locally issued / peer-issued CID tracking with sequence numbers, `active_connection_id_limit` enforcement per RFC9000 § 18.2, NEW_CONNECTION_ID validation per § 19.15 with retire_prior_to handling, RETIRE_CONNECTION_ID processing per § 19.16, queued outbound retires, peer-active promotion gate); `QuicAntiAmplificationLimit` (RFC9000 § 8.1 per-path 3x send-budget accounting with validated-state cap removal); and `QuicPathSet`/`QuicPath`/`QuicPathState` (RFC9000 § 9 per-address path state with Probing/Validating/Validated/Primary/Abandoned transitions, PATH_CHALLENGE token tracking, validated-promotion-removes-cap, and demote-previous-primary-on-promotion semantics). Driver integration (NEW_CONNECTION_ID emission after handshake, active peer CID swap on promotion, anti-amplification gate on outbound builders, server migration lifecycle) is the remaining P1 driver-integration step.
...
- Native QUIC still needs broader recovery soak/backoff validation and full per-address path migration driver integration: issuing NEW_CONNECTION_ID after handshake, switching the active peer CID on path promotion, gating outbound packet builders on the anti-amplification budget, and server-side migration lifecycle. Basic PATH_CHALLENGE token handling, CID inventory primitives, anti-amplification primitives, and 1-RTT CID routing are no longer active gaps.
