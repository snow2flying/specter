...
- Native QUIC now has send-time tracking, ACK-driven RTT/PTO estimator updates, client Handshake plus server Initial/Handshake CRYPTO PTO retransmission, event-level peer close draining, Retry/VN client-handshake handling, and client PATH_CHALLENGE/PATH_RESPONSE token lifecycle coverage.
...
- Native H3 TLS now advertises certificate compression from the TLS fingerprint, controls deterministic-vs-browser-permuted extension behavior, emits raw ordered QUIC transport parameters when configured, and has low-level session-ticket/0-RTT helpers plus `NativeH3SessionCache` while keeping high-level resumption/0-RTT capabilities explicitly unsupported until fully wired into H3 connection establishment.
...
- Wired native H3 TLS extension-order behavior into BoringSSL permutation control, added session-ticket capture/install helpers, `NativeH3SessionCache`, and 0-RTT early-data context setup, and kept native H3 TLS capability status explicit that full resumption and 0-RTT remain unsupported until the high-level H3 path is wired.
...
- TLS certificate compression, deterministic-vs-browser-permuted extension behavior, raw ordered QUIC transport parameters, low-level session-ticket helpers, `NativeH3SessionCache`, and 0-RTT early-data context setup are wired for native H3; remaining fingerprint work is explicit extension-list ordering, high-level resumption/0-RTT integration, capture presets, and dynamic placeholders.
- TLS resumption and 0-RTT are now both explicit and partially plumbed at the helper/cache layer; the remaining gap is wiring `NativeH3SessionCache` into H3Client/transport, replay policy, and transport send integration, not ambiguity.
...
- Native QUIC still needs full packet-space RFC9002 PTO/timer-driven recovery with PTO backoff, client Initial PTO replay, RFC-grade close-drain timing beyond the bounded client replay window, ECN socket marking plus CE-driven congestion response, and path migration/validation beyond the client token lifecycle.
...
- TLS/H3 fingerprint gaps remain: explicit extension-list ordering beyond BoringSSL permutation policy, wiring `NativeH3SessionCache` into H3Client with `SSL_SESSION` reuse policy, 0-RTT send with replay policy, capture-derived raw transport-parameter presets, and dynamic connection-ID placeholder handling inside raw ordered transport-parameter lists.
