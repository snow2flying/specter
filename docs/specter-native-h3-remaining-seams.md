...
- Attempted broader `CARGO_TARGET_DIR=/tmp/specter-h3-test-target cargo test --test h3_native_quic ack --no-default-features -- --nocapture`; the current `h3_native_quic` target does not compile because concurrent tests reference not-yet-present `retry_integrity_tag_v1`, `validate_retry_integrity_tag_v1`, and `QuicPathValidator`.
