...
- QUIC send-time tracking, ACK-driven RTT/PTO estimator updates, client Initial/Handshake CRYPTO PTO retransmission, server Initial/Handshake CRYPTO PTO retransmission, client application-space PTO timer/retransmit, server application ACK-driven recovery and PTO STREAM retransmit core, mock/same-fixture server application loss-detection wake integration, and event-level peer close draining are implemented; remaining recovery work is broader recovery soak/backoff validation and close-drain timer/retransmit behavior.
